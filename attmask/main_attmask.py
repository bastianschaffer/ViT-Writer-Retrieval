...
def get_args_parser():
    ...
    parser.add_argument('--local_crops_scale', type=float, nargs='+', default=(0.05, 0.4), # type=float, nargs='+'
        help="""Scale range of the cropped image before resizing, relatively to the origin image.
    ...
    parser.add_argument("--num_channels", default=3, type=int,
                        help="Number of channels to use for the Vit (rgb -> 3, grayscale -> 1).")
                        
    return parser

def my_collate_fn(batch):    
    # commentend print-statements to see whats going on with "batch":
    #
    # print("-------------------- collate batch type: ", type(batch))
    # print("len(batch): ", len(batch)) # 30 (batch size)
    # print("type(batch[0]):", type(batch[0])) # tuple (return-type of dataset -> data and labels)
    # print("type(batch[0][0]): ", type(batch[0][0])) # list
    # print("len(batch[0][0]):", len(batch[0][0])) # 10 (number of augmentations)
    # print("type(batch[0][0][0]):", type(batch[0][0][0])) # tensor
    # print("batch[0][0][0].shape:", batch[0][0][0].shape) # [1, 224, 224] (or [3, 224, 224] for RGB)

    crops_num = len(batch[0][0])
    num_channels = batch[0][0][0].shape[0]

    batch_size = len(batch)
    before = time.time()

    new_batch = [torch.empty((batch_size, num_channels, 224, 224)) if i < 2 else torch.empty((batch_size, num_channels, 96, 96)) for i in range(crops_num)]

    labels = torch.tensor([0 for _ in range(batch_size)]) # dummy label because we dont need them
    masks = []
    for b in range(batch_size):
        ...
        for i in range(crops_num):
            start = time.time()
            new_batch[i][b] = next_win[i]

    return (new_batch, labels) # ,masks)

def train_attmask(run, args):
    ...
    #)
    transform_train = My_Data_Augmentation(
        args.global_crops_scale,
        args.local_crops_scale,
        args.global_crops_number,
        args.local_crops_number,
    ...
    #dataset_train = ImageFolderMask(
    #    data_path=args.data_path, # here we use the path of the train split of imagenet
    #    transform=transform_train,
    #    patch_size=pred_size,
    #    pred_ratio=args.pred_ratio,
    #    pred_ratio_var=args.pred_ratio_var,
    ...
    """
    print("----------------- args in channels: ", args.num_channels)
    ...
    trainset_len = len(data_loader_train)
    ...
    if args.load_from:
        ...
        print("NOT loading models from checkpoint!")
    start_epoch = to_restore["epoch"]
    print("Start epoch: ", start_epoch)
    print("trainset len: ", trainset_len)
   ...
   # log_stats.update(knn_results)
    ...
    torch.multiprocessing.set_sharing_strategy('file_system') # i (Basti) added this line to try to fix a bug
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(run, student, teacher, teacher_without_ddp, ibot_loss,
            data_loader_train, optimizer, lr_schedule, wd_schedule, momentum_schedule,
            epoch, fp16_scaler, transform_train, args)
...
def train_one_epoch(run, student, teacher, teacher_without_ddp, ibot_loss, data_loader,
                    optimizer, lr_schedule, wd_schedule, momentum_schedule,epoch,
                    fp16_scaler, transform_train,  args):
    ...
    avg_delta = 0
    avg_loss = 0
    loss_list = [0 for i in range(2*50)]
    
    #for it_, (images, labels, masks) in enumerate(data_loader):
    for it_, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        loss_list = [loss.item()] + loss_list[:-1]

        if it == 0:
            # TODO it is global iteration -> use it_ for batch iteration
            avg_loss = loss.item()
        else:
            avg_loss = avg_loss * (it_) / ((it_+1)) + loss.item() / (it_+1)
...
class My_Data_Augmentation(object):
    def __init__(self, global_crops_scale, local_crops_scale, global_crops_number, local_crops_number):
        ...
        self.global_transfo1 = transforms.Compose([
            ...
            transforms.Lambda(self.random_dilation_erosion),
            normalize
        ])
    ...
    def random_kernel(self):
        ...
        num_ones = random.randint(1, 4)
        ones_positions = random.sample(range(9), num_ones)
        for pos in ones_positions:
            kernel[pos // 3, pos % 3] = 1
        return kernel


    def random_dilation_erosion(self, img):
        img = np.array(img)
        if random.random() < 0.5:
            kernel = self.random_kernel()
...
def train_attmask_wrapper(sweep_config):
    """Wrapper function to parse args and call train_attmask."""
    parser = argparse.ArgumentParser('AttMask', parents=[get_args_parser()])
    args = parser.parse_args()


    run = wandb.init(project="hpc-attmask")
    for key, value in wandb.config.items():
        setattr(args, key, value)
    
    args.local_crops_scale = [float(x) for x in args.local_crops_scale.split(' ')]
    args.global_crops_scale = [float(x) for x in args.global_crops_scale.split(' ')]
    train_attmask(run, args)

    

if __name__ == '__main__':
    # code for sweeping with wandb:
    #with open('sweep-config.yml', 'r') as f:
    #    sweep_config = yaml.safe_load(f)
    #sweep_id = wandb.sweep(sweep_config, project="hpc-attmask")
    #wandb.agent(sweep_id, function=lambda: train_attmask_wrapper(sweep_config), count=None)
