...
class ImageFolderMask(ImageFolder):
    def __init__(self, *args, data_path, transform, patch_size, pred_ratio, pred_ratio_var, pred_aspect_ratio, 
                 pred_shape='block', pred_start_epoch=0, **kwargs):
        #super(ImageFolderMask, self).__init__(*args, **kwargs)
        self.dataset = MyDataset(
            data_path,
            transform)
        self.samples = self.dataset.task_map
    ...
    def __getitem__(self, index):
        output, label = self.dataset[index]
